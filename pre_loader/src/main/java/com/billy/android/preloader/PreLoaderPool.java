...
public class PreLoaderPool {
    ...
    public boolean refresh(int id, String keyInGroup) {
        WorkerGroup worker = (WorkerGroup) workerMap.get(id);
        return worker != null && worker.refresh(keyInGroup);
    }

    public GroupedDataLoader getDataLoaderByKeyInGroup(int id, String keyInGroup) {
        WorkerGroup iWorker = (WorkerGroup) workerMap.get(id);
        return iWorker.getDataLoader(keyInGroup);
    }
