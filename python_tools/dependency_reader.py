...
linePattern3 = re.compile(r'rootProject\.ext\.deps\.[^\.]+')
...
class Dependency:
    ...
    def __init__(self, root_dir):
        ...
        self.modules = {}
        ### 匹配当前groupId的依赖项
        self.pattern = re.compile(r'.*ompile.*((' + gradle_properties.get('maven_groupId') + ')|(maven_groupId)|(rootProject\.ext\.deps.)).*')
    ...
    def get_all_reverse_dependencies(self, name):
        ...
        return rev_modules
    #获取直接或间接依赖name的所有module（若是snapshot的则不再继续获取后续依赖）
    def get_all_reverse_dependencies_4_deploy(self, name, gradle_properties, maven_type):
        rev_modules = []
        find_reverse_dependency_module_4_deploy(self.modules, name, rev_modules, gradle_properties, maven_type)
        #按照全局排序的顺序来进行deploy
        result = []
        for i in self.sorted_modules:
            if i in rev_modules:
                result.append(i)
        return result
...
def read_dependency_line(line):
    ...
    if ',' in line: # 类似于： compile group:maven_groupId, name:'lib_module_c', version: lib_module_c
        p = linePattern
    elif 'rootProject.ext.deps' in line:
        type = 3
        p = linePattern3
    else:
        type = 2
...
### 反向查找指定module的依赖（直接和间接依赖该module的module）
def find_reverse_dependency_module_4_deploy(modules, name, rev_modules, gradle_properties, maven_type):
    for i in modules:
        if name in modules[i]:
            if i not in rev_modules:
                rev_modules.append(i)
            already_snapshot = gradle_properties.get(gradle_properties.get('version_prefix') + i).endswith('SNAPSHOT')
            if already_snapshot and maven_type.lower() == 'snapshot':
                print 'module:' + i + ' is already snapshot'
            else:
                find_reverse_dependency_module_4_deploy(modules, i, rev_modules, gradle_properties, maven_type)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        gradle_version_properties = property.parse(os.path.join(project_abs_path, 'gradle.properties'))
        deploy_maven_type = 'snapshot'
        if len(sys.argv) > 2:
            deploy_maven_type = sys.argv[2]
        revert_dependencies = dependency.get_all_reverse_dependencies_4_deploy(sys.argv[1], gradle_version_properties, deploy_maven_type)
        print 'revert dependencies:'
        print revert_dependencies
    else:
